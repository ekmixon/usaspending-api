...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.zip_filepath = f'{self.destination}_{datetime.utcnow().strftime("%Y%m%d%H%M_utc")}.zip'
        ...
        logger.info(f"Success! New zip file: {self.zip_filepath}")
        logger.info(f"Script completed in {perf_counter() - script_start}s")
    ...
    def gather_tas_from_file(self):
        ...
        for header in tas_code_header:
            headers.extend(cell.value.replace("\n", "").strip() for cell in header)
        if expected_headers != headers:
            raise Exception(f"Headers {headers} Don't match expected: {expected_headers}")

        tas_code_rows = ws["A9":"G100"]
        return [
            {
                key: cell.value.strip() if cell.value else None
                for key, cell in itertools.zip_longest(headers, row)
            }
    ...
    def query_database(self, tas_dict_list):
        ...
        for i, tas in enumerate(tas_dict_list, 1):
            ...
            filepath = f"{self.temporary_dir}/{generate_tas_rendering_label(tas)}_fpds.csv"
            dump_to_csv(filepath, contract_results)

            assistance_results = self.single_tas_query(tas, "assistance")
            filepath = f"{self.temporary_dir}/{generate_tas_rendering_label(tas)}_fabs.csv"
        ...
        logger.info(f"Completed all database queries in {perf_counter() - db_start}s")

    def single_tas_query(self, tas_dict, transaction_type="contract"):
        single_db_query = perf_counter()
        sql_string = CONTRACT_SQL if transaction_type == "contract" else ASSISTANCE_SQL
        formatted_dict = {
            k: f"= '{v}'" if v else "IS NULL" for k, v in tas_dict.items()
        }
        ...
        if self.verbose:
            ...
            logger.info(
                f"Query for {transaction_type}s using above TAS took {perf_counter() - single_db_query}s returning {len(results)} rows"
            )
    ...
    def assemble_csv_files(self):
        ...
        logger.info(
            f"Assembling data and saving to final .csv files took {perf_counter() - start_pandas}s"
        )
    ...
    def write_pandas(self, df, award_type):
        ...
        if self.verbose:
            logger.info(f"Completed pandas dataframe for all {award_type} records")

        for quarter in pd.unique(df["_fyq"]):
            filepath = f"{self.destination}/{quarter}_{award_type}.csv"
